...
afterEvaluate {
    ...
    val buildTool = if (file("${frontendDir}/pnpm-lock.yaml").exists()) "pnpm" else "yarn"
    ...
    tasks.register("buildFrontend") {
        ...
        doLast {
            context.exec.exec {
                ...
                commandLine(Frontend.commandLine(listOf(buildTool, "install")))
            }

            val command = if (buildTool == "pnpm") mutableListOf("pnpm", "run", "build") else mutableListOf("yarn", "run", "build")
